 ...
 * 
 * @param i2c_addr The I2C address of the INA3221 device. Default is INA3221_DEFAULT_ADDRESS.
 * @param theWire The I2C bus to use. Default is &Wire.
 * @return bool True if initialization and configuration were successful, false otherwise.
 */
bool Adafruit_INA3221::begin(uint8_t i2c_addr, TwoWire *theWire) {
    ...
    if (getManufacturerID() != INA3221_MANUFACTURER_ID || getDieID() != INA3221_DIE_ID) {
        return false;
    }
    ...
    if (!setBusVoltageConvTime(INA3221_CONVTIME_1MS) || !setShuntVoltageConvTime(INA3221_CONVTIME_1MS)) {
        return false;
    }
 ...
 * @return float The shunt voltage in volts. Returns NaN if the channel is invalid.
 */
float Adafruit_INA3221::getShuntVoltage(uint8_t channel) {
    ...
    float voltage = (rawValue >> 3) * 40e-6;  // Drop bottom 3 bits and multiply by 40uV
 ...
 * 
 * @param channel The channel number (0, 1, or 2).
 * @return float The bus voltage in volts. Returns NaN if the channel is invalid.
 */
float Adafruit_INA3221::getBusVoltage(uint8_t channel) {
    ...
    // Convert raw value to voltage
    float voltage = (rawValue >> 3) * 8e-3;  // Drop bottom 3 bits and multiply by 8mV
...
uint16_t Adafruit_INA3221::getManufacturerID() {
    Adafruit_BusIO_Register manufacturerID_reg(i2c_dev, INA3221_REG_MANUFACTURER_ID, 2, MSBFIRST);
...
bool Adafruit_INA3221::reset() {
    Adafruit_BusIO_Register config_reg(i2c_dev, INA3221_REG_CONFIGURATION, 2, MSBFIRST);
    Adafruit_BusIO_RegisterBits rst_bit(&config_reg, 1, 15);  // RST is the 15th bit in the Configuration Register
...
bool Adafruit_INA3221::enableChannel(uint8_t channel) {
    ...
    Adafruit_BusIO_Register config_reg(i2c_dev, INA3221_REG_CONFIGURATION, 2, MSBFIRST);
    Adafruit_BusIO_RegisterBits channel_bit(&config_reg, 1, 14 - channel);  // Channel enable bits are 12-14 in the Configuration Register
...
bool Adafruit_INA3221::setAveragingMode(ina3221_avgmode mode) {
    Adafruit_BusIO_Register config_reg(i2c_dev, INA3221_REG_CONFIGURATION, 2, MSBFIRST);
    Adafruit_BusIO_RegisterBits avg_bits(&config_reg, 3, 9);  // AVG2-0 bits are 9-11 in the Configuration Register
 ...
 * 
 * @return ina3221_avgmode The current averaging mode. Possible return values are:
 * - INA3221_AVG_1_SAMPLE
 * - INA3221_AVG_4_SAMPLES
 * - INA3221_AVG_16_SAMPLES
 * - INA3221_AVG_64_SAMPLES
...
bool Adafruit_INA3221::setShuntVoltageConvTime(ina3221_convtime convTime) {
    Adafruit_BusIO_Register config_reg(i2c_dev, INA3221_REG_CONFIGURATION, 2, MSBFIRST);
    Adafruit_BusIO_RegisterBits vshct_bits(&config_reg, 3, 3);  // VSHCT 2-0 bits are 3-5 in the Configuration Register
 ...
 * 
 * @return ina3221_convtime The current shunt voltage conversion time. Possible return values are:
 * - INA3221_CONVTIME_140US
 * - INA3221_CONVTIME_204US
 * - INA3221_CONVTIME_332US
 * - INA3221_CONVTIME_588US
...
bool Adafruit_INA3221::setBusVoltageConvTime(ina3221_convtime convTime) {
    Adafruit_BusIO_Register config_reg(i2c_dev, INA3221_REG_CONFIGURATION, 2, MSBFIRST);
    Adafruit_BusIO_RegisterBits vbusct_bits(&config_reg, 3, 6);  // VBUSCT 2-0 bits are 6-8 in the Configuration Register
 ...
 * 
 * @return ina3221_convtime The current bus voltage conversion time. Possible return values are:
 * - INA3221_CONVTIME_140US
 * - INA3221_CONVTIME_204US
 * - INA3221_CONVTIME_332US
 * - INA3221_CONVTIME_588US
...
bool Adafruit_INA3221::setMode(ina3221_mode mode) {
    Adafruit_BusIO_Register config_reg(i2c_dev, INA3221_REG_CONFIGURATION, 2, MSBFIRST);
    Adafruit_BusIO_RegisterBits mode_bits(&config_reg, 3, 0);  // MODE [3-1] bits are 0-2 in the Configuration Register
 ...
 * @return bool True if the operation was successful, false otherwise.
 */
bool Adafruit_INA3221::setCriticalAlertThreshold(uint8_t channel, float current) {
    if (channel > 2) return false;  // Invalid channel

    int16_t threshold = static_cast<int16_t>(current * _shunt_resistance[channel] * 8);  // Convert to int16_t, considering the 3 LSBs
    uint16_t reg_addr = INA3221_REG_CRITICAL_ALERT_LIMIT_CH1 + channel;
 ...
 * 
 * @param channel The channel number (0-2).
 * @return float The current threshold in Amperes. Returns NaN for invalid channel.
 */
float Adafruit_INA3221::getCriticalAlertThreshold(uint8_t channel) {
    if (channel > 2) return NAN;  // Invalid channel

    uint16_t reg_addr = INA3221_REG_CRITICAL_ALERT_LIMIT_CH1 + channel;
    Adafruit_BusIO_Register alert_reg(i2c_dev, reg_addr, 2, MSBFIRST);

    int16_t threshold = alert_reg.read();
    return static_cast<float>(threshold) / (_shunt_resistance[channel] * 8);  // Convert back to Amperes
}
 ...
 * @return bool True if the operation was successful, false otherwise.
 */
bool Adafruit_INA3221::setWarningAlertThreshold(uint8_t channel, float current) {
    if (channel > 2) return false;  // Invalid channel
 ...
 * 
 * @return uint16_t The flag indicators. Use the defined bit flags to check specific flags.
 * 
...
uint16_t Adafruit_INA3221::getFlags() {
    Adafruit_BusIO_Register mask_enable_reg(i2c_dev, INA3221_REG_MASK_ENABLE, 2, MSBFIRST);
    uint16_t flags = mask_enable_reg.read();

    // Mask to keep only the relevant flag bits
    uint16_t mask = INA3221_CONV_READY | INA3221_TIMECONT_ALERT | INA3221_POWER_VALID | INA3221_WARN_CH3 | INA3221_WARN_CH2 | 
                    INA3221_WARN_CH1 | INA3221_SUMMATION | INA3221_CRITICAL_CH3 | INA3221_CRITICAL_CH2 | INA3221_CRITICAL_CH1;
...
bool Adafruit_INA3221::setSummationChannels(bool ch1, bool ch2, bool ch3) {
    ...
    Adafruit_BusIO_RegisterBits scc_bits(&mask_enable_reg, 3, 12);  // 3 bits starting from bit 12
...
bool Adafruit_INA3221::setPowerValidLimits(float vlimitlow, float vlimithigh) {
  ...
  Adafruit_BusIO_Register low_limit_reg_obj =
      Adafruit_BusIO_Register(i2c_dev, INA3221_REG_POWERVALID_LOWERLIMIT, 2, MSBFIRST);
  ...
  Adafruit_BusIO_Register high_limit_reg_obj =
      Adafruit_BusIO_Register(i2c_dev, INA3221_REG_POWERVALID_UPPERLIMIT, 2, MSBFIRST);
